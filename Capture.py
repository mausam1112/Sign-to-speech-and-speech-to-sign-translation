...
def create_folder(folder_name):
    if not os.path.exists('H:/mydata/training_set/' + folder_name):
        os.mkdir('H:/mydata/training_set/' + folder_name)
    if not os.path.exists('H:/mydata/test_set/' + folder_name):
        os.mkdir('H:/mydata/test_set/' + folder_name)
    ...
    for loop in listImage:
        while True:
            ...
            if cv2.waitKey(1) == ord('c'):
                ...
                if t_counter <= 350:
                    img_name = "H:/mydata/training_set/" + str(ges_name) + "/{}.png".format(training_set_image_name)
                ...
                if t_counter > 350 and t_counter <= 400:
                    img_name = "H:/mydata/test_set/" + str(ges_name) + "/{}.png".format(test_set_image_name)
                    save_img = cv2.resize(mask, (image_x, image_y))
                    cv2.imwrite(img_name, save_img)
                    print("{} written!".format(img_name))
                    test_set_image_name += 1
